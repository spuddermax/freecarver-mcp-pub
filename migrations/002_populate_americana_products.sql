...
BEGIN
  ...
  INSERT INTO products (name, description, price, product_media)
  SELECT 
    'Carved ' ||
  ...
  -- For 5 random newly inserted products, insert one product option with an associated option SKU
  FOR rec IN (
      ...
      -- Insert a random product option with a unique name (includes the product id)
      ...
      -- Insert a variant for the new option
      ...
      -- Insert a random SKU for that product option variant
      INSERT INTO product_option_skus (product_id, option_id, variant_id, sku, price)
      VALUES (
          rec.product_id,
          new_option_id,
          new_variant_id,
          'SKU-' || rec.product_id || '-' || floor(random()*10000)::int,
