...
export function UserPersonalDetails({
	...
	const getPhoneNumberColor = () => {
		...
		return isValidUSPhoneNumber(User.phoneNumber)
			? "text-green-500"
			: "text-red-500";
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (User.phoneNumber && !isValidUSPhoneNumber(User.phoneNumber)) {
			onMessage({
				type: "error",
				text: "Please enter a valid US phone number in the format (XXX) XXX-XXXX",
			});
			setLoading(false);
			return;
		}
		...
		try {
			...
			onMessage({
				type: "error",
				text: "Error updating personal details. Please try again.",
			});
		} finally {
			setLoading(false);
		}
				...
				<div className="space-y-4">
					...
					<div>
						...
						<div className="mt-1 relative">
							<div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
								<Phone
									className={`h-5 w-5 ${getPhoneNumberColor()}`}
								/>
							</div>
							<input
								type="tel"
