  ...
  for (const otherPoint of otherPoints) {
    ...
    for (let i = 1; i < otherPoint.trail.length; i++) {
      ...
      if (futureDistance < 20) { // Wider threshold for future collisions
        ...
        const clockwiseDistance = calculateTurnDistance(point, clockwise, trailSegment);
        const counterClockwiseDistance = calculateTurnDistance(point, counterClockwise, trailSegment);
        
        point.direction = (clockwiseDistance > counterClockwiseDistance ? clockwise : counterClockwise) as Point['direction'];
...
function calculateTurnDistance(point: Point, newDirection: number, segment: { x1: number, y1: number, x2: number, y2: number }) {
  ...
  const dot = ((futureX - segment.x1) * dx + (futureY - segment.y1) * dy) / (len * len);
