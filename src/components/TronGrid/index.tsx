...
export function TronGrid() {
	...
	useEffect(() => {
		...
		// Initialize points
		const availableColors: Point["color"][] = [
			...GRID_CONSTANTS.COLORS_USED,
		];
		const playerColors = Array.from(
			{ length: GRID_CONSTANTS.NUM_DOTS },
			(_, i) => availableColors[i % availableColors.length]
		);
		pointsRef.current = Array.from(
			{ length: GRID_CONSTANTS.NUM_DOTS },
			(_, index) => ({
				x: Math.random() * canvas.width,
				y: Math.random() * canvas.height,
				direction: GRID_CONSTANTS.DIRECTIONS[
					Math.floor(Math.random() * GRID_CONSTANTS.DIRECTIONS.length)
				] as Point["direction"],
				speed:
					GRID_CONSTANTS.SPEED.MIN +
					Math.random() *
						(GRID_CONSTANTS.SPEED.MAX - GRID_CONSTANTS.SPEED.MIN),
				progress: 0,
				trail: [],
				active: true,
				respawnTime: null,
...
function updatePoint(
	...
	if (!point.active && point.respawnTime !== null) {
		if (currentTime >= point.respawnTime) {
			...
			point.direction = GRID_CONSTANTS.DIRECTIONS[
	...
	if (Math.random() < 0.01) {
		...
		const newDirection = ((currentDirection + turn + 360) %
			360) as Point["direction"];
