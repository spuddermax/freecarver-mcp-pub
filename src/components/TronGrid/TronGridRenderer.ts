...
export function drawExplosion(ctx: CanvasRenderingContext2D, x: number, y: number, progress: number, point: Point) {
...
export function drawPoint(ctx: CanvasRenderingContext2D, point: Point, currentTime: number) {
  // Draw trail
  if (point.trail.length > 0) {
    // Calculate the direction vector of the dot
    const directionRad = (point.direction * Math.PI) / 180;
    const trailOffsetX = -Math.cos(directionRad) * (GRID_CONSTANTS.DOT.LENGTH / 2); // Offset by half the dot length in opposite direction
    const trailOffsetY = -Math.sin(directionRad) * (GRID_CONSTANTS.DOT.LENGTH / 2);
