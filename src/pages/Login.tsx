...
export default function Login({ onLogin }: LoginProps) {
	...
	const ConnectionIcon = () => {
		switch (connectionStatus) {
			...
			case "error":
				return (
					<DatabaseOff className="h-5 w-5 text-red-600 dark:text-red-400" />
				);
			default:
				return (
					<DatabaseBackup className="h-5 w-5 text-gray-600 dark:text-gray-400 animate-pulse" />
				);
		}
	};
