...
router.post("/", async (req, res) => {
	try {
		...
		if (!option_name) {
			...
			return res
				.status(400)
				.json({ error: '"option_name" is required.' });
		}
		const result = await pool.query(
...
router.get("/:id", async (req, res) => {
	try {
		...
		res.status(200).json({ option: result.rows[0] });
	} catch (error) {
		logger.error(
...
variantsRouter.post("/", async (req, res) => {
	try {
		...
		if (!option_value) {
			...
			return res
				.status(400)
				.json({ error: '"option_value" is required.' });
		}
		const result = await pool.query(
...
variantsRouter.get("/:variantId", async (req, res) => {
	try {
		...
		res.status(200).json({ variant: result.rows[0] });
	} catch (error) {
		logger.error(
